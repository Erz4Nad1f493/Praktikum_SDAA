...
stack<barang*> stack_undo; // Stack untuk menyimpan barang yang dihapus (Undo)
queue<barang*> queue_barang; // Queue untuk barang yang baru diterima
...
void hapus_barang(barang** head, int* jumlah_barang) {
    ...
    if (temp != NULL && temp->id == id) {
        *head = temp->next;
        stack_undo.push(temp); // Simpan ke Stack untuk undo
        (*jumlah_barang)--;
